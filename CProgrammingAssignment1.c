...
int main(void) {
    ...
    //               우리 대학에서 가장 긴 학과 이름까지 넣을 수 있도록 13비트를 할당함
    
    printf("본인의 이름을 입력하세요: ");
    scanf("%s", name); // string형으로 초기화한 name 변수에 입력한 이름을 할당함
    //                    string형 입력이므로 %s를 사용
    getchar(); // 혹시 모를 입력 버퍼 방지용 getchar()
    
    printf("생일을 입력하세요: "); // scanf 후 자동 줄바꿈을 고려해 \n을 넣지 않음
    scanf("%d", &birthday); // int형으로 초기화한 birthday 변수에 입력한 생일을 할당함
    //                         int형 입력이므로 %d를 사용
    getchar();
    
    printf("학번을 입력하세요: ");
    scanf("%d", &stud_id); // int형으로 초기화한 stud_id 변수에 입력한 학번을 할당함
    getchar();
    
    printf("학과를 입력하세요: ");
    scanf("%s", dept); // 마지막으로, string형으로 초기화한 dept 변수에 입력한 학과를 할당함
    getchar();
